...
(() => {
    ...
    async function addCustomSelector(host, selector) {
        ...
        const cur = await getSettings();
        const map = cloneObj(cur.customSelectorsByHost);

        const arr = Array.isArray(map[host]) ? map[host].slice() : [];
        if (!arr.includes(selector)) arr.push(selector);

        map[host] = arr;
        return setSettingsPatch({ customSelectorsByHost: map });
    }
    ...
    async function removeCustomSelector(host, selector) {
        ...
        // boş kaldıysa host’u komple silebilirsin; şimdilik dursun.
        return setSettingsPatch({ customSelectorsByHost: map });
    }
    ...
    function pickBestRateFromSettings(settings, host, from, to, opts = {}) {
        ...
        if (pinned) return { ...pinned, source: "PINNED" };
        ...
        if (m != null) return { rate: m, confidence: 1.0, evidence: "manual", source: "MANUAL_HOST" };
        ...
        if (sitePairs) {
            ...
            const invK = pairKey(to, from);
            const inv = sitePairs[invK];
        ...
        if (legacy != null) return { rate: legacy, confidence: 1.0, evidence: "legacy-manual", source: "MANUAL_LEGACY" };
    ...
    globalThis.SCCStorage = {
        ...
        getSettings,
        setSettingsPatch,
        updateSettings, // ✅ NEW (opsiyonel ama faydalı)
