...
(() => {
    ...
    function readRateFromSelector(doc, selector, meta = {}) {
        ...
        for (let i = 0; i < nodes.length && i < maxNodes; i++) {
            ...
            for (const txt of candidates) {
                if (!looksRatey(txt, quote)) continue;
        ...
        sccDbg(`[rateSel] host=${host} quote=${quote} base=${base} sel="${selector}" matches=${nodes.length} FAIL`);
        return null;
    }

    function inferFromRateSelectors(doc, host, settings, baseCode) {
        ...
        if (entry.USD) {
            const v = readRateFromSelector(doc, entry.USD, { host, quote: "USD", base });
            if (v != null) put("USD", v);
        }
        if (entry.EUR) {
            const v = readRateFromSelector(doc, entry.EUR, { host, quote: "EUR", base });
            if (v != null) put("EUR", v);
        }
    ...
    function detectSiteRates(doc, host, settings, opts = {}) {
        ...
        const fromSelectors = inferFromRateSelectors(doc, host, settings, baseCode);
